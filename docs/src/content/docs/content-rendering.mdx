...
By default, atoms persist their state to `localStorage`. This means that when a user returns to your site, they immediately see the last personalized content instead of the fallback — even before the new fetch completes. Persistent atoms also synchronize across browser tabs, so a content update in one tab is reflected in all other tabs showing the same atom.
...
Set the `sticky` option to `false` to disable persistence. The atom uses an ephemeral in-memory store instead, and every page load starts from the fallback content:
...
Atoms automatically refresh when user behavior changes if the `auto-refresh-atom` plugin is enabled. Importing `croct` from `croct-nanostores` registers the plugin before `croct.plug()` runs, and the Croct SDK enables registered plugins by default:
...
When a tracked event fires, the library doesn't refresh immediately. Instead, it uses a cascade strategy aligned with Croct's SLAs for event propagation. Since a single event on the server can trigger a chain of derived events (for example, a `userSignedUp` event may update user attributes, which in turn may match a new audience), the library starts refreshing after 500 ms and then repeats every 500 ms for the full propagation window:

1. **First refresh** at 500 ms after the event
2. **Ten additional refreshes** every 500 ms while derived events propagate
3. **Final refresh** 5.5 seconds after the triggering event

Each refresh captures any newly derived state, ensuring personalized content fully reflects the latest user data by the time the cascade completes. If the content didn't change across refreshes, no subscriber will be called and no component will re-render.
